	...
	private class RepoLoader extends AsyncTask<Void, FileObject, Void> {
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				GHRepository ghRepository = gitHub.getRepository("mxc19912008/Key-Algorithms");
		...
		private boolean isFileType(FileObject fileObject) {
			if ( fileObject.getAbsoluteFilePath().endsWith(".java") || fileObject.getAbsoluteFilePath().endsWith(".gradle") || fileObject.getAbsoluteFilePath()
